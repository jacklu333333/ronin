...
from .model_temporal import BilinearLSTMSeqNetwork, LSTMSeqNetwork, TCNSeqNetwork
from .transformations import ComposeTransform, RandomHoriRotateSeq
from .utils import MSEAverageMeter, load_config
...
def train(args, **kwargs):
    ...
    history = (
        network.get_receptive_field() if args.type == "tcn" else args.window_size // 2
    )
    ...
    if args.continue_from is not None and osp.exists(args.continue_from):
        with open(
            osp.join(str(Path(args.continue_from).parents[1]), "config.json"), "r"
        ) as f:
        ...
        if device.type == "cpu":
            ...
            checkpoints = torch.load(
                args.continue_from, map_location={model_data["device"]: args.device}
            )
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for bid, batch in enumerate(train_loader):
                ...
                train_vel.add(
                    predicted.cpu().detach().numpy(), targ.cpu().detach().numpy()
                )
            ...
            if val_loader:
                ...
                for bid, batch in enumerate(val_loader):
                    ...
                    pred = network(feat)
                    val_vel.add(
                        pred.cpu().detach().numpy(), targ.cpu().detach().numpy()
                    )
...
def recon_traj_with_preds_global(
    dataset, preds, ind=None, seq_id=0, type="preds", **kwargs
):
    ind = (
        ind
        if ind is not None
        else np.array([i[1] for i in dataset.index_map if i[0] == seq_id], dtype=np.int)
    )
...
if __name__ == "__main__":
    ...
    default_config_file = osp.abspath(
        osp.join(osp.abspath(__file__), "../../config/temporal_model_defaults.json")
    )

    import argparse

    parser = argparse.ArgumentParser(
        description="Run seq2seq model in train/test mode [required]. Optional "
    ...
    # common
    parser.add_argument(
        "--type", type=str, choices=["tcn", "lstm", "lstm_bi"], help="Model type"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    tcn_cmd.add_argument(
        "--channels", type=str, help="Channel sizes for TCN layers (comma separated)"
    )
    # lstm
