...
class HeadingNetwork(torch.nn.Module):
    ...
    def consistent_find_leftmost(self, values, condition):
        indices = torch.arange(
            values.size(1), 0, -1, dtype=torch.float, device=values.device
        )
    ...
    def get_moving_mask(self, values, threshold):
        ...
        min_i = (
            self.consistent_find_leftmost(values, lambda x: x >= threshold)
            .float()
            .unsqueeze(-1)
        )
        indices = (
            torch.arange(0, values.size(1), dtype=torch.float, device=values.device)
            .unsqueeze(0)
            .expand_as(values)
        )
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            if not quiet_mode:
                print("-" * 25)
                print(
                    "Epoch {}, time usage: {:.3f}s, loss {} result: {} ".format(
...
def test(args, **kwargs):
    ...
    for idx, data in enumerate(test_data_list):
        ...
        if args.out_dir is not None and osp.isdir(args.out_dir):
            np.save(
                osp.join(args.out_dir, "{}_{}_heading.npy".format(data, "lstm")),
                np.concatenate([pred_heading, gt_heading], axis=1),
            )
        ...
        print(
            "{} :- heading mse: {:.3f} angle: {:.3f} norm: {:.3f}".format(
                data, heading_mse[-1], heading_angles[-1], norm_err
            )
        )
        ...
        if not args.fast_test:
            ...
            plt.plot(angle)
            plt.legend(
                "mse: {:.3f}, angl: {:.3f}".format(heading_mse[-1], heading_angles[-1])
            )
        ...
        if args.use_trajectory_type == "gt":
            ...
            if osp.exists(
                osp.join(
                    args.out_dir, "{}_{}.npy".format(data, args.use_trajectory_type)
                )
            ):
                traj = np.load(
                    osp.join(
                ...
                raise ValueError(
                    "Trajectory file {}_{}.npy is missing".format(
                        data, args.use_trajectory_type
                    )
                )
        ...
        p_l = {"m": "--", "c": "b"}
        handles = [
            mpatch.Patch(color=g_l["c"], label="Ground_truth"),
    ...
    values = format_string(np.mean(heading_mse), np.mean(heading_angles), sep="\t")
    print(measure, "\n", values)
...
if __name__ == "__main__":
    ...
    """
    default_config_file = osp.abspath(
        osp.join(osp.abspath(__file__), "../../config/heading_model_defaults.json")
    )

    import argparse

    parser = argparse.ArgumentParser(
        description="Run seq2seq heading model in train/test mode [required]. Optional "
    ...
    parser.add_argument(
        "--feature_sigma", type=float, help="Gaussian for smoothing features"
    )
    parser.add_argument(
        "--target_sigma", type=float, help="Gaussian for smoothing target"
    )
    ...
    test_cmd.add_argument(
        "--prefix", type=str, default="", help="prefix to add when saving files."
    )
    test_cmd.add_argument(
        "--use_trajectory_type",
        type=str,
    ...
    if (
        args.mode == "train"
        and kwargs.get("weights")
        and type(kwargs.get("weights")) != list
    ):
