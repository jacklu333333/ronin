...
from baselines.data_stabilized_local_speed import (
...
class CvSVR(SVRBase):
    ...
    def fit(self, feature, target):
        self.m.train(
            feature.astype(np.float32), cv2.ml.ROW_SAMPLE, target.astype(np.float32)
        )
    ...
    def load(self, model_dir):
        self.m = cv2.ml.SVM_load(
            osp.join(model_dir, "regressor_{}.yaml".format(self.chn))
        )
...
def get_dataset(root_dir, data_list, args, **kwargs):
    ...
    total_mem = (
        len(dataset)
        * (args.window_size * _feature_dim + _target_dim)
        * 32
        / 1024
        / 1024
        / 1024
...
def get_dataset_from_list(root_dir, list_path, args, **kwargs):
    with open(list_path) as f:
        ...
        data_list = [
            d[0] for d in list_all if args.attachment is None or d[1] == args.attachment
        ]
    return get_dataset(root_dir, data_list, args, **kwargs)


def train_chn(args):
    features, targets, _ = get_dataset_from_list(
...
def train(args):
    ...
    if args.config is not None:
        ...
        es = [
            config["chn_{}".format(i)]["param"]["epsilon"]
...
def grid_search(args):
    ...
    for i in range(targets.shape[1]):
        ...
        best_params["chn_{}".format(i)] = {
            "param": grid_searcher.best_params_,
...
def recon_traj_with_preds(dataset, preds, seq_id=0, **kwargs):
    ...
    rot_grav_to_glob = (ori * quaternion.from_float_array(rot_local_to_grav).conj())[
        ind
    ]

    za = np.zeros(preds.shape[0])
    preds_q = quaternion.from_float_array(
        np.stack([za, preds[:, 0], za, preds[:, 1]], axis=1)
    )
    glob_v = (
        quaternion.as_float_array(rot_grav_to_glob * preds_q * rot_grav_to_glob.conj())[
            :, 1:
        ]
        * dt
    )
...
def test_sequence(args):
    if args.test_path is not None:
        ...
        with open(args.test_list) as f:
            ...
            data_list = [
                d[0]
                for d in list_all
                if args.attachment is None or d[1] == args.attachment
            ]
    else:
        raise ValueError
    ...
    mean, std = 0.0, 1.0
    models = [CvSVR(i, 0.0, 0.0) for i in range(dummy_targ.shape[1])]
    ...
    for data in data_list:
        ...
        if args.align_length is not None:
            _, r, t = fit_transformation(
                pos_pred[: args.align_length], pos_gt[: args.align_length]
            )
        ...
        print(
            "{}: loss: {}/{:.6f}, ate: {:.6f}, rte: {:.6f}".format(
                data, loss, np.mean(loss), ate, rte
            )
        )
    ...
    print(
        "Overall loss: {}/{:.6f}, avg ATE: {:.6f}, avg RTE: {:.6f}".format(
            np.mean(losses_all, axis=0),
            np.mean(losses_avg),
            np.mean(ate_all),
            np.mean(rte_all),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, default="train", choices=["train", "test", "search"]
    )
