...
if __name__ == "__main__":
    ...
    for data in data_list:
        ...
        step_v = np.stack(
            [np.zeros(step.shape[0]), np.ones(step.shape[0]) * args.stride], axis=1
        )

        step_glob = np.squeeze(
            np.matmul(rot_hori, np.expand_dims(step_v, axis=2)), axis=2
        )
    ...
    print(
        "All done. Average ate:{:.6f}, average rte: {:.6f}".format(
            np.mean(ate_all), np.mean(rte_all)
        )
    )
    if args.out_dir is not None and osp.isdir(args.out_dir):
