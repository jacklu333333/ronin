...
def train(args, **kwargs):
    ...
    end_t = time.time()
    print(
        "Training set loaded. Feature size: {}, target size: {}. Time usage: {:.3f}s".format(
    ...
    if args.val_list is not None:
        val_dataset = get_dataset_from_list(
            args.root_dir, args.val_list, args, mode="val"
        )
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            print("-------------------------")
            print(
                "Epoch {}, time usage: {:.3f}s, average loss: {}/{:.6f}".format(
...
def test_sequence(args):
    ...
    for data in test_data_list:
        ...
        print(
            "Sequence {}, loss {} / {}, ate {:.6f}, rte {:.6f}".format(
                data, losses, np.mean(losses), ate, rte
            )
        )
        ...
        if args.out_dir is not None and osp.isdir(args.out_dir):
            np.save(
                osp.join(args.out_dir, data + "_gsn.npy"),
    ...
    if args.out_dir is not None and osp.isdir(args.out_dir):
        with open(osp.join(args.out_dir, "losses.csv"), "w") as f:
            ...
            for i in range(losses_seq.shape[0]):
                ...
                f.write(
                    "{:.6f},{:6f},{:.6f}\n".format(
                        losses_avg[i], ate_all[i], rte_all[i]
                    )
                )

    print(
        "----------\nOverall loss: {}/{}, avg ATE:{}, avg RTE:{}".format(
            np.average(losses_seq, axis=0),
            np.average(losses_avg),
            np.mean(ate_all),
            np.mean(rte_all),
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default="ronin", choices=["ronin", "ridi"]
    )
