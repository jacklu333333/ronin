...
def compile_annotated_sequence(root_dir, data_list, args):
    ...
    for data in data_list:
        try:
            ...
            imu_time_offset = np.loadtxt(
                osp.join(data_path, "processed/imu_time_offset.txt")
            )
            body_align = np.loadtxt(
                osp.join(data_path, "processed/align_tango_to_body.txt")
            )
            device = "unknown"
            ...
            meta_info = {
                ...
                "date": date,
                "device": device,
                "imu_reference_time": np.loadtxt(
                    osp.join(data_path, "imu/reference_time.txt")
                ).tolist(),
                "tango_reference_time": np.loadtxt(
                    osp.join(data_path, "tango/reference_time.txt")
                ).tolist(),
            ...
            with h5py.File(osp.join(out_path, "data.hdf5"), "x") as f:
                ...
                for source in _raw_data_sources:
                    f.create_dataset(
                        "raw/tango/" + source,
                        data=np.genfromtxt(
                            osp.join(data_path, "tango/" + source + ".txt")
                        ),
                    )
                for source in _optional_data_sources:
                ...
                f.create_group("pose")
                corrected_tango = np.load(
                    osp.join(data_path, "processed/corrected_tango.npy")
                )
    ...
    for data in data_list:
        try:
            ...
            reference_time = np.loadtxt(
                osp.join(data_path, "reference_time.txt")
            ).tolist()
            ...
            for source in all_sources.keys():
                if source in source_vector:
                    ...
                    processed_sources[source] = process_data_source(
                        all_sources[source][:, [0, 4, 1, 2, 3]],
                        output_time,
                        "quaternion",
                    )

            init_gyro_bias = (
                processed_sources["gyro_uncalib"][0] - processed_sources["gyro"][0]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type", type=str, choices=["annotated", "unannotated"], default="annotated"
    )
