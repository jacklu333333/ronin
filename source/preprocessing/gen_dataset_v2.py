...
def process_data_source(raw_data, output_time, method):
    ...
    if method == "vector":
        output_data = interpolate_vector_linear(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", type=str, default=None, help="Path to a dataset folder."
    )
    parser.add_argument(
        "--skip_front",
        type=int,
    ...
    parser.add_argument(
        "--skip_end", type=int, default=200, help="Numbef of discarded records at end"
    )
    parser.add_argument(
        "--output_samplerate",
        type=int,
        default=200,
        help="Output sample rate. Default is 200Hz",
    )
    parser.add_argument(
        "--recompute",
        action="store_true",
        ...
        if os.path.exists(data_root + "/processed/data.csv") and not args.recompute:
            ...
            for source in source_all:
                try:
                    ...
                    source_data[:, 0] = (
                        source_data[:, 0] - reference_time
                    ) / _nano_to_sec
                    all_sources[source] = source_data
                except OSError:
                    print(
                        "Can not find file for source {}. Please check the dataset.".format(
                            source
                        )
                    )
                    exit(1)
            ...
            for src_id, src in all_sources.items():
                print(
                    "Source: %s,  start time: %d, end time: %d"
                    % (src_id, src[0, 0], src[-1, 0])
                )
            ...
            for source in all_sources.keys():
                ...
                if source in source_vector:
                    processed_source[source] = process_data_source(
                    ...
                    all_sources[source][:, [1, 2, 3, 4]] = all_sources[source][
                        :, [4, 1, 2, 3]
                    ]
                    processed_source[source] = process_data_source(
                        all_sources[source], output_time, "quaternion"
                    )
                print(
                    "{} found. Input sampling rate: {}Hz. Channel size:{}".format(
                        source, input_sr, processed_source[source].shape[1]
                    )
                )
            ...
            def add_pose_data(key_id, output_name):
                processed_source[output_name + "_pos"] = np.zeros(
                    [output_time.shape[0], 3]
                )
                processed_source[output_name + "_ori"] = np.zeros(
                    [output_time.shape[0], 4]
                )
                if key_id in all_sources:
                    ...
                    print(
                        "{} found. Input sampling rate: {}Hz".format(
                            output_name, input_sr
                        )
                    )
                    processed_source[output_name + "_pos"] = process_data_source(
                        pose_data, output_time, "vector"
                    )
                    processed_source[output_name + "_ori"] = process_data_source(
                        ori_data, output_time, "quaternion"
                    )
            ...
            data_mat = np.concatenate(
                [
                    ...
                    processed_source["tango_adf_ori"],
                ],
                axis=1,
            )
            ...
            if not args.fast_mode and not args.no_trajectory and "pose" in all_sources:
                tango_pos = processed_source["tango_pos"][
                    args.skip_front : -args.skip_end
                ]
                tango_ori = processed_source["tango_ori"][
                    args.skip_front : -args.skip_end
                ]
                print("Writing trajectory to ply file")
                write_ply_to_file(
                    path=output_folder + "/trajectory.ply",
                    position=tango_pos,
                    orientation=tango_ori,
                    num_axis=0,
                )

                if osp.exists(osp.join(args.path, "pose_adf.txt")):
                    tango_adf_pos = processed_source["tango_adf_pos"][
                        args.skip_front : -args.skip_end
                    ]
                    tango_adf_ori = processed_source["tango_adf_ori"][
                        args.skip_front : -args.skip_end
                    ]
                    print("Writing adf trajectory to ply file")
                    write_ply_to_file(
                        path=output_folder + "/trajectory_adf.ply",
                        position=tango_adf_pos,
                        orientation=tango_adf_ori,
                        trajectory_color=(200, 128, 255),
                        num_axis=0,
                    )
    print(
        "All done. Total length: {:.2f}s ({:.2f}min)".format(
